...
export default function RoundIntro({ gameState, myId, lang, onDone }) {
  ...
  let roleIcon, roleLine, roleHint;
  if (isTransmitter) {
    roleIcon = '📡';
    roleLine = lang === 'pt' ? 'VOCÊ TRANSMITE A DICA' : "YOU SEND THE CLUE";
    roleHint = lang === 'pt' ? 'Gire a roleta · Escolha a pista' : 'Spin the wheel · Give the clue';
  } else {
    roleIcon = '🎯';
    roleLine = lang === 'pt' ? 'VOCÊ CALIBRA O PAINEL' : "YOU CALIBRATE THE PANEL";
    roleHint = lang === 'pt' ? 'Ouça a pista · Ajuste o ponteiro' : 'Hear the clue · Adjust the needle';
  }
    ...
    <div onClick={dismiss} style={{
      ...
      <div style={{
        ...
        <div style={{ height:4, background:'rgba(255,255,255,0.07)' }}>
          <div style={{ height:'100%', width:`${progress}%`, background:color, boxShadow:`0 0 8px ${color}`, transition:'none' }}/>
        </div>

        <div style={{ padding:'28px 28px 24px', display:'flex', flexDirection:'column', alignItems:'center', gap:20, textAlign:'center' }}>

          {/* Round */}
          <div>
            <div className="label mb-6" style={{ color:'var(--ink-dim)' }}>
              {lang==='pt'?'RODADA':'ROUND'} {(gameState.round ?? 0) + 1} / {gameState.totalRounds ?? '?'}
            </div>
          </div>

          {/* Transmitter */}
          <div style={{ width:'100%', padding:'14px 0', borderTop:'1px solid rgba(255,255,255,0.07)', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
            <div className="label mb-4" style={{ color:'var(--ink-dim)' }}>📡 {lang==='pt'?'TRANSMISSOR':'TRANSMITTER'}</div>
            <div style={{ fontFamily:'var(--f-vt)', fontSize:36, color:'var(--neon-amber)', lineHeight:1 }}>
              {transmitter?.name ?? '?'}
            </div>
            {transmitter?.isBot && (
              <div style={{ fontFamily:'var(--f-body)', fontSize:11, color:'var(--ink-dim)', marginTop:3 }}>
                (age automaticamente)
              </div>
            )}
          </div>
          ...
          {/* Role */}
          <div style={{ width:'100%', padding:'12px 16px', borderRadius:8, border:`2px solid ${color}`, background:`${color}07` }}>
